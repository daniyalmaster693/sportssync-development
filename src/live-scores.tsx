...
export default function Command() {
  ...
  const { isLoading: isLoadingNHL, data: nhlScoresAndSchedule } = useFetch(
  ...
  useEffect(() => {
    if (selectedGameIndex !== null && nhlGames[selectedGameIndex]) {
      updateTitle(selectedGameIndex);
    }
  }, [nhlGames, selectedGameIndex]);

  const updateTitle = async (index) => {
    const nhlGame = nhlGames[index];
    if (!nhlGame) return;

    let title = `${nhlGame.shortName} - ${new Date(nhlGame.date).toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
    })}`;

    if (nhlGame.status.type.state === "in") {
      title = `${nhlGame.competitions[0].competitors[1].team.abbreviation} ${nhlGame.competitions[0].competitors[1].score} - ${nhlGame.competitions[0].competitors[0].team.abbreviation} ${nhlGame.competitions[0].competitors[0].score}   Q${nhlGame.status.period} ${nhlGame.status.displayClock}`;
    }
  ...
  return (
    <MenuBarExtra isLoading={isLoadingNHL} icon={Icon.Livestream} title={menuBarTitle}>
      ...
      <MenuBarExtra.Submenu title="Basketball Games">
        <MenuBarExtra.Submenu title="NBA Games">
          <MenuBarExtra.Section title={`${nbaGamesDate}`}></MenuBarExtra.Section>
          {nbaItems}
        </MenuBarExtra.Submenu>

        <MenuBarExtra.Submenu title="WNBA Games">
          <MenuBarExtra.Section title={`${wnbaGamesDate}`}></MenuBarExtra.Section>
          {wnbaItems}
        </MenuBarExtra.Submenu>
      </MenuBarExtra.Submenu>

      <MenuBarExtra.Submenu title="College Basketball Games">
        <MenuBarExtra.Submenu title="MNCAA Games">
          <MenuBarExtra.Section title={`${mncaaGamesDate}`}></MenuBarExtra.Section>
          {mncaaItems}
        </MenuBarExtra.Submenu>

        <MenuBarExtra.Submenu title="WNCAA Games">
          <MenuBarExtra.Section title={`${wncaaGamesDate}`}></MenuBarExtra.Section>
          {wncaaItems}
        </MenuBarExtra.Submenu>
      </MenuBarExtra.Submenu>

      <MenuBarExtra.Submenu title="Football Games">
        <MenuBarExtra.Submenu title={`NFL Games`}>{nflItems}</MenuBarExtra.Submenu>

        <MenuBarExtra.Submenu title={`NCAA Games`}>{ncaaItems}</MenuBarExtra.Submenu>
      </MenuBarExtra.Submenu>
